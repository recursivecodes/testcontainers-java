...
public class CmdModifierTest {
    ...
    // hostname {
    @Rule
    public GenericContainer theCache = new GenericContainer<>(DockerImageName.of("redis:3.0.2"))
            .withCreateContainerCmdModifier(cmd -> cmd.withHostName("the-cache"));
    // }

    // memory {
    @Rule
    public GenericContainer memoryLimitedRedis = new GenericContainer<>(DockerImageName.of("redis:3.0.2"))
